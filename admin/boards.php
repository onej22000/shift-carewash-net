            ...
            <?php foreach ($boardPosts[$boardType] as $post): ?>
                ...
                <?php if ($editBoardType === $boardType && $editPostId === (int) $post['id']): ?>
                    <form method="post" action="/admin/board_action.php" class="board-form">
                        ...
                        <div class="board-form-actions"><button type="submit">更新する</button> <a href="/admin/boards.php#board-<?= htmlspecialchars($boardType, ENT_QUOTES, 'UTF-8') ?>">キャンセル</a></div>
                    </form>
                <?php else: ?>
                    ...
                    <div class="board-post-meta">
                        ...
                        <span class="board-post-actions">
                            <a href="/admin/boards.php?edit_post=<?= (int) $post['id'] ?>&board_type=<?= htmlspecialchars($boardType, ENT_QUOTES, 'UTF-8') ?>#board-<?= htmlspecialchars($boardType, ENT_QUOTES, 'UTF-8') ?>">編集</a>
                            <form method="post" action="/admin/board_action.php" class="inline-form" onsubmit="return confirm('この投稿を削除します。よろしいですか？');">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>">
