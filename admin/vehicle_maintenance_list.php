        ...
        if ($action === 'delete') {
            ...
            if ($recordStmt->fetchColumn() === false) {
                ...
                try {
                    ...
                    record_vehicle_maintenance_history($pdo, $recordId, 'delete', (int) $admin['id'], 'admin', $before, null);
                    $pdo->commit();
                    set_flash('success', '車両管理記録を削除しました。');
                    header('Location: /admin/vehicle_maintenance_list.php?vehicle_id=' . $filterVehicleId);
                    exit;
                } catch (\Throwable $e) {
                    $pdo->rollBack();
            ...
            if (!empty($parseErrors)) {
                ...
                record_vehicle_maintenance_history($pdo, $recordId, 'create', (int) $admin['id'], 'admin', null, build_vehicle_maintenance_snapshot($pdo, $recordId));

                set_flash('success', '車両管理記録を登録しました。');
                header('Location: /admin/vehicle_maintenance_list.php?vehicle_id=' . $values['vehicle_id']);
                exit;
            } else {
                $recordId = (int) ($_POST['id'] ?? 0);
                $recordStmt = $pdo->prepare('SELECT id FROM vehicle_maintenance WHERE id = :id AND deleted_at IS NULL');
                ...
                if ($recordStmt->fetchColumn() === false) {
                    ...
                    try {
                        ...
                        record_vehicle_maintenance_history($pdo, $recordId, 'update', (int) $admin['id'], 'admin', $before, build_vehicle_maintenance_snapshot($pdo, $recordId));
                        $pdo->commit();
...
<header>
    <h1>車両管理記録</h1>
    <nav>ログイン中: <?= htmlspecialchars($admin['name'], ENT_QUOTES, 'UTF-8') ?>さん（管理者） | <a href="/admin/vehicles.php">車両マスタ管理</a> | <a href="/admin/vehicle_check_list.php">集荷前車両等チェック記録</a> | <a href="/admin/vehicle_alert_settings.php">アラート設定</a> | <a href="/admin/dashboard.php">ダッシュボード</a> | <a href="/admin/logout.php">ログアウト</a></nav>
</header>
...
<form method="get" action="/admin/vehicle_maintenance_list.php" class="filter-row">
...
<section class="maintenance-form">
    ...
    <?php else: ?>
    <fieldset>
        <form method="post" action="/admin/vehicle_maintenance_list.php">
            ...
            <p style="margin-top:10px;">
                ...
                <?php if ($formAction === 'update'): ?>
                    <a href="/admin/vehicle_maintenance_list.php?vehicle_id=<?= $filterVehicleId ?>">キャンセル</a>
                <?php endif; ?>
            </p>
        </form>
    </fieldset>
            ...
            <tbody>
                <?php foreach ($records as $record): ?>
                    <tr>
                        ...
                        <td>
                            <a href="/admin/vehicle_maintenance_list.php?vehicle_id=<?= $filterVehicleId ?>&edit=<?= (int) $record['id'] ?>">編集</a>
                            <form method="post" action="/admin/vehicle_maintenance_list.php" class="inline-form" onsubmit="return confirm('この記録を削除しますか？');">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>">
