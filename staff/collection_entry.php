        ...
        if ($action === 'record_consumable_transaction') {
            ...
            if (!in_array($facilityId, $validFacilityIds, true)) {
                ...
                if ($isCleaningSite) {
                    ...
                    $dispatchCycleIdsRaw = $_POST['dispatch_cycle_ids'] ?? [];
                    if (!is_array($dispatchCycleIdsRaw)) {
                        $dispatchCycleIdsRaw = [];
                    }
                    $dispatchBagCountsRaw = $_POST['dispatch_bag_counts'] ?? [];
                    if (!is_array($dispatchBagCountsRaw)) {
                        $dispatchBagCountsRaw = [];
                    }
                    $dispatchCycleIds = [];
                    foreach ($dispatchCycleIdsRaw as $rawCycleId) {
                        $cycleId = (int) $rawCycleId;
                        if ($cycleId > 0 && !in_array($cycleId, $dispatchCycleIds, true)) {
                            $dispatchCycleIds[] = $cycleId;
                        }
                    }
                    ...
                    foreach ($dispatchCycleIds as $dispatchCycleId) {
                        if (!in_array($dispatchCycleId, $validDispatchIds, true)) {
                            $dispatchSelectionError = '発送対象のサイクルが無効です（既に他の記録で更新された可能性があります）。もう一度やり直してください。';
                            break;
                        }
                        $dispatchBagCount = parse_bag_count($dispatchBagCountsRaw[$dispatchCycleId] ?? '');
                        if ($dispatchBagCount === null) {
                            $targetName = '';
                            foreach ($dispatchCandidates as $candidate) {
                                if ((int) $candidate['id'] === $dispatchCycleId) {
                                    $targetName = $facilityNamesById[(int) $candidate['facility_id']] ?? '';
                                    break;
                                }
                            }
                            $dispatchSelectionError = ($targetName !== '' ? $targetName . 'の' : '選択した対象の') . '発送リネン袋数を入力してください。';
                            break;
                        }
                        $dispatchEntries[] = [
                            'cycle_id' => $dispatchCycleId,
                            'bag_count' => $dispatchBagCount,
                        ];
                    }
                    ...
                    if ($dispatchSelectionError !== '') {
                        ...
                        try {
                            foreach ($dispatchEntries as $dispatchEntry) {
                                update_dispatch(
                                    $pdo,
                                    $dispatchEntry['cycle_id'],
                                    $dispatchEntry['bag_count'],
                                    $dispatchDate,
                                    $dispatchTime,
                                    $dispatchEmployeeId,
                                    $facilityId
                        ...
                        $parts = [];
                        if ($wantsDispatch) {
                            $dispatchTotalBagCount = array_sum(array_column($dispatchEntries, 'bag_count'));
                            if (count($dispatchEntries) === 1) {
                                $parts[] = 'クリーニング所発送（' . $dispatchTotalBagCount . '袋）';
                            } else {
                                $parts[] = 'クリーニング所発送（' . count($dispatchEntries) . '件・計' . $dispatchTotalBagCount . '袋）';
                            }
                        }
                        if ($wantsArrival) {
        ...
        <form method="post" action="/staff/collection_entry.php">
            ...
            <?php if ($step2['is_cleaning_site']): ?>
                <fieldset<?= empty($step2['dispatch_candidates']) ? ' class="disabled"' : '' ?>>
                    ...
                    <?php else: ?>
                        <p class="notice">発送する施設を複数選択できます。チェックした施設を1回の操作でまとめて発送登録します。</p>
                        <?php if (count($step2['dispatch_candidates']) > 1): ?>
                            <p class="candidate-actions">
                                <button type="button" id="dispatch-select-all">すべて選択</button>
                                <button type="button" id="dispatch-clear-all">選択解除</button>
                            </p>
                        <?php endif; ?>
                        <?php foreach ($step2['dispatch_candidates'] as $cycle): ?>
                            <?php
                            $dispatchCycleId = (int) $cycle['id'];
                            $dispatchBagCountDefault = $cycle['return_ready_bag_count'] ?? '';
                            $dispatchCheckedByDefault = count($step2['dispatch_candidates']) === 1;
                            ?>
                            <div class="candidate-row dispatch-candidate-row<?= $dispatchCheckedByDefault ? ' is-selected' : '' ?>">
                                <label class="dispatch-candidate-label">
                                    <input
                                        type="checkbox"
                                        class="dispatch-cycle-checkbox"
                                        name="dispatch_cycle_ids[]"
                                        value="<?= $dispatchCycleId ?>"
                                        <?= $dispatchCheckedByDefault ? 'checked' : '' ?>
                                    >
                                    <?= format_cycle_candidate_label($cycle, $facilityNamesById) ?>
                                </label>
                                <div class="form-row dispatch-bag-row">
                                    <label for="dispatch_bag_count_<?= $dispatchCycleId ?>">発送リネン袋数</label>
                                    <input
                                        type="number"
                                        id="dispatch_bag_count_<?= $dispatchCycleId ?>"
                                        class="dispatch-bag-count"
                                        name="dispatch_bag_counts[<?= $dispatchCycleId ?>]"
                                        min="0"
                                        step="1"
                                        value="<?= htmlspecialchars((string) $dispatchBagCountDefault, ENT_QUOTES, 'UTF-8') ?>"
                                        <?= $dispatchCheckedByDefault ? '' : 'disabled' ?>
                                    >
                                </div>
                            </div>
                        <?php endforeach; ?>
            ...
            <p class="notice">日付・時間は現在日時、担当者はログイン中のご本人（<?= htmlspecialchars($staff['name'], ENT_QUOTES, 'UTF-8') ?>）を初期値にしています。必要に応じて変更できます。発送はチェックした対象だけを記録し、選択した対象のリネン袋数は必須です。</p>
...
document.querySelectorAll('.dispatch-candidate-row').forEach(function (row) {
    var checkbox = row.querySelector('.dispatch-cycle-checkbox');
    if (checkbox) {
        checkbox.addEventListener('change', function () {
            syncDispatchCandidate(row);
        });
    }
    syncDispatchCandidate(row);
});

var dispatchSelectAllButton = document.getElementById('dispatch-select-all');
if (dispatchSelectAllButton) {
    dispatchSelectAllButton.addEventListener('click', function () {
        document.querySelectorAll('.dispatch-candidate-row').forEach(function (row) {
            var checkbox = row.querySelector('.dispatch-cycle-checkbox');
            if (checkbox) {
...
var dispatchClearAllButton = document.getElementById('dispatch-clear-all');
if (dispatchClearAllButton) {
    dispatchClearAllButton.addEventListener('click', function () {
        document.querySelectorAll('.dispatch-candidate-row').forEach(function (row) {
            var checkbox = row.querySelector('.dispatch-cycle-checkbox');
            if (checkbox) {
